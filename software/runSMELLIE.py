...
def readConfigFile(config_id):
        ...
        safeLaserSwitchOutput = jsoncmd.getConfigPara(config_id,"safe_laser_switch_output")
        safeFibreSwitchInput = jsoncmd.getConfigPara(config_id,"safe_fibre_switch_input")
        safeFibreSwitchOutput = jsoncmd.getConfigPara(config_id,"safe_fibre_switch_output")
     
        global fibreSwitchSerialPort, fibreSwitchBaudRate
        fibreSwitchSerialPort = jsoncmd.getConfigPara(config_id,"fibre_switch_serial_port")
...
def timeout(connection, timeout_sec):
	...
	while 1:
		try:
			item_set = connection.recv(tcpipMaxStringLength)
		except conn.timeout:
...
def main():	
	...
	try:
		while 1:
    			start_run = connection.recv(tcpipMaxStringLength)
    			if (start_init_data == start_initialise_flag):      # check if initialisation command has been given
