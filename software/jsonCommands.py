...
def getRunParameters(run):
    file = '../daq/SMELLIE_very_short.json'
    dict = ReadJSON(file)
...
def getConfigPara(configId,parameterName):
    nameList,valueList = getConfigurationParameters(configId)
    if parameterName in nameList:
        indexValue = nameList.index(parameterName)
        return valueList[indexValue]
    else:
        sys.exit("Not a correct JSON Parameter.\nAttempted to use ConfigID: " + str(configId) + " and ParameterName: " + str(parameterName))         
