...
def SetIOChannels(input_channel, output_channel):
    ...
    if (input_channel < 1) or (input_channel > 5):
        print "FibreSwitch (SetIOChannels) - The input channel is not valid ... it must be between 1 and 5 inclusive."

    # invalid output_channel values will not be accepted
    if (output_channel < 1) or (output_channel > 14):
        print "FibreSwitch (SetIOChannels) - The output channel is not valid ... it must be between 1 and 14 inclusive."
