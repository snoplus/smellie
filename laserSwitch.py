...
def GetSelectedChannel():
    def invert(bit):
        if (bit == 0):
            ...
            return "Invalid input - check connections to and from the Laser Switch."
...
def SetSelectedChannel(picked_channel):
    if (picked_channel >= 6) or (picked_channel < 0):
        print "The channel number is not valid - it must be between 0 and 5 inclusive."
        return 0 
    else:
...
def CheckExecution():
    ...
    if (manual_execution_flag == 1):
	    ...
	    print "Execution not in Progress"
	    return  
